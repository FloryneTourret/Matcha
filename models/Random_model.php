...
class Random_Model extends Model
{
    public function random_bio()
    {
        $bio = array(
    ...
    public function random_last_name()
    {
        $last_names = array(
    ...
    public function random_first_name($gender = null)
    {
    ...
    {
        $stmt = $this->db->prepare("SELECT tag_id FROM tags");
        $stmt->execute();
        $tags = $stmt->fetchAll();
    ...
    public function get_address($latitude, $longitude)
    {
        ...
        curl_setopt_array($curl, array(
            CURLOPT_URL => "https://maps.googleapis.com/maps/api/geocode/json?latlng=" . $latitude . "," . $longitude . "&key=AIzaSyCFUDkSZ_ocdopTHNoZiZeq7Uq8T8ARhM4",
    ...
    public function random_picture($gender = null, $login)
    {
        if($gender == 1)
            $gender = "Male";
        else if ($gender == 2)
        ...
        if (!file_exists( 'assets/upload/' . $login)) {
            mkdir( 'assets/upload/' . $login, 0777, true);
        }
        $filename = 'assets/upload/' . $login . '/' . uniqid() . ".jpg";
        copy($picture, $filename);

        return ($filename);
    }

    public function random_username($gender = null)
    {
        if ($gender == 1)
            $gender = "Male";
        else if ($gender == 2)
